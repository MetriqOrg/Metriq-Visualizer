...
class OffscreenGeometryRenderer:
    ...
    def render(self, current_time: float) -> np.ndarray:
        ...
        elev = float(self.options.elev)
        azim = float(self.options.azim)
        if bool(self.options.autorotate):
            azim += max(0.0, float(current_time) - float(getattr(self.options, "start_time", 0.0))) * float(self.options.rotation_speed)
        zoom = float(self.options.zoom)
