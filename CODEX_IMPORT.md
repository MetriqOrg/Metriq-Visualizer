...
This archive contains the complete Metriq Visualizer 1.12.8 source tree. It can be opened independently or merged into an existing clone while preserving `.git`, branches, tags, remotes, and unrelated files.

## Safe merge

From the extracted v1.12.8 folder:
...
git switch -c feature/metriq-visualizer-1.12.8
git status --short
git diff --stat
git diff
git diff --check
...
```bash
git add -A
git commit -m "Release Metriq Visualizer 1.12.8"
git push -u origin feature/metriq-visualizer-1.12.8
```
